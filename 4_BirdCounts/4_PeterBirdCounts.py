...
# This program: Allows a user to input a bird name regardless of capitalization 
...
def askUser(d):
    ...
    ## breaks out of the continuous loop started in main()
    elif inquiry == "exit":
        print "Good bye.\n"
        quit()   
    else:
...
def mostCount():
    d = countBirds(filename)
    mostCount = max([i for i in d.values()])
    return mostCount

# Allow the user to enter a bird name as often as the like.
# When they want to stop entering bird names they can type 'Exit'.
def main():
    # Creates a continuous loop which can be broken in askUser(d)
    e = 0
    while e == 0:  
        # establishes a dictionary of tallied bird sightings from file 
        d = countBirds(filename)
        # allows user interface and prints requested data
        print askUser(d)
        # prints extra data for user re: most seen birds
        print "The following bird have been seen the most (%i) times: %s\n" % (mostCount(), mostSeen(d))
